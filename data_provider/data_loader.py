...
class cafe(Dataset):
    ...
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='outputog.csv',
                 target='value', scale=True, freq='w',seasonal_patterns=None,timeenc=0):
    ...
    def last_insample_window(self):
        ...
        insample = np.zeros((len(self.timeseries.T), self.seq_len))
        insample_mask = np.zeros((len(self.timeseries.T), self.seq_len))
        for i, ts in enumerate(self.timeseries.T):
            ts_last_window = ts[-self.seq_len:]
            insample[i, -len(ts):] = ts_last_window
            insample_mask[i, -len(ts):] = 1.0
            return insample, insample_mask